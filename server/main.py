...
def get_runs(tournament_id: int, team_uuid: str | None = None, db: Session = Depends(get_db)):
    ...
    if team_uuid is not None:
        run_list = [run for run in run_list if team_uuid in [submission_run.submission.team.team_uuid if submission_run.submission is not None else None for submission_run in run.submission_run_infos]]
